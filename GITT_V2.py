...
class GITTApp:
    ...
    def _build_ui(self):
        ...
        TAB_SECTIONS = [
            ...
            # Analysis (before W&H and K&C)
            ('ocv',            'OCV',                   'anal'),
            ('relax_curves',   'Relaxation curves',     'anal'),
            ('relax_delta',    'Relaxation dV',         'anal'),
    ...
    def _replot(self,*_):
        ...
        try:
            ...
            pidx2 = max(0, self._pulse_var.get()-1)
    ...
    def _draw_all_pulses(self,active,tfs,tfe):
        ...
        for name in active:
            s=self._samples[name]
            for ax,cyc,cmap_name in [(ax_d,'discharge','Blues_r'),(ax_c,'charge','Oranges')]:
                bl=[b for b in s['blocks'] if b['cycle']==cyc]
                if not bl: continue
                # V_eq range for colour mapping
                veqs=[b.get('V_eq',np.nan) for b in bl]
                veq_arr=np.array([v for v in veqs if np.isfinite(v)])
                vmin=veq_arr.min() if len(veq_arr) else 0.
                vmax=veq_arr.max() if len(veq_arr) else 1.
                ...
                for b in bl:
                    ...
                    c=cmap(norm(veq)) if np.isfinite(veq) else '#aaa'
                    ax.plot(sq[nz],dV[nz],'-',lw=0.8,color=c,alpha=0.6)
                    if fm.sum()>=2:
                        sl=b['sl_sqrt']
                        if np.isfinite(sl):
                            xf=np.linspace(sq[fm].min(),sq[fm].max(),50)
                            ic=np.mean(dV[fm])-sl*np.mean(sq[fm])
                            ax.plot(xf,sl*xf+ic,'--',lw=1.2,color=c,alpha=0.85)
                sm=cm.ScalarMappable(cmap=cmap,norm=norm); sm.set_array([])
                cb=fig.colorbar(sm,ax=ax,fraction=0.03,pad=0.02)
                cb.set_label(r'$V_\mathrm{eq}$ (V)',fontsize=8)
        for ax,lbl in [(ax_d,'Discharge'),(ax_c,'Charge')]:
            ...
            ax.set_xlabel(r'$\sqrt{t}$ (s$^{1/2}$)',fontsize=10)
            ax.set_ylabel(r'$\Delta V$ (V)',fontsize=10)
            ax.set_title(lbl,fontsize=10,fontweight='bold',color='#1a6b35')
            ax.grid(True,alpha=0.25)
        fig.suptitle(f'All pulses — sqrt(t) space  |  colour = V_eq  |  fit window [{tfs:.0f}-{tfe:.0f} s]',
                     fontsize=10,fontweight='bold',color='#1a6b35')
        fig.tight_layout(); self._flush('all_pulses')
    ...
    def _draw_sqrt_fits(self,active,tfs,tfe,pulse_idx=None):
        ...
        for ci,name in enumerate(active):
            ...
            for row,(blist,cyc_lbl) in enumerate([(disc_bl,'Discharge'),(chg_bl,'Charge')]):
                ...
                ax.set_ylabel(r'$\Delta V$ (V)',fontsize=8)
                veq=b.get('V_eq',np.nan)
    ...
    def _draw_all_xi(self, active, tau):
        """All K&C xi pulses overlaid, coloured by V_eq — mirror of _draw_all_pulses."""
        import matplotlib.cm as cm, matplotlib.colors as mcolors
        ...
        for name in active:
            s = self._samples[name]
            for ax, cyc, cmap_name in [
                    (ax_d, 'discharge', 'Blues_r'),
                    (ax_c, 'charge',    'Oranges')]:
                bl = [b for b in s['blocks'] if b['cycle'] == cyc
                      and len(b['_x_kc']) >= 2]
                if not bl: continue
                veqs = [b.get('V_eq', float('nan')) for b in bl]
                valid_v = [v for v in veqs if v == v]
                ...
                for b in bl:
                    ...
                    import math
                    c = cmap(norm(veq)) if math.isfinite(veq) else '#aaa'
                    ax.plot(xkc, Vr, '-', lw=0.8, color=c, alpha=0.6)
                    win = (xkc >= xi_lo) & (xkc <= xi_hi) if xi_hi > xi_lo \
                          else [True]*len(xkc)
                    win = win if hasattr(win, '__len__') else list(win)
                    n_win = sum(bool(w) for w in win)
                    if n_win >= 2 and math.isfinite(b.get('sl_kc', float('nan'))):
                        from scipy import stats as _st2
                        xw = xkc[win]; Vw = Vr[win]
                        sl_w, ic_w, *_ = _st2.linregress(xw, Vw)
                        xf = [xw.min() + (xw.max()-xw.min())*i/59 for i in range(60)]
                        import numpy as _np2
                        xf = _np2.array(xf)
                        ax.plot(xf, sl_w*xf + ic_w, '--', lw=1.2, color=c, alpha=0.85)
                sm = cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([])
                cb = fig.colorbar(sm, ax=ax, fraction=0.03, pad=0.02)
                cb.set_label('V_eq (V)', fontsize=8)

        for ax, lbl in [(ax_d, 'Discharge'), (ax_c, 'Charge')]:
            ax.invert_xaxis()
            if xi_hi < 99. or xi_lo > 0.:
                ax.axvspan(xi_lo, xi_hi, alpha=0.07, color='#8a2c2c')
            ax.set_xlabel('xi = sqrt(t+tau)-sqrt(t)  (s^0.5)', fontsize=10)
            ax.set_ylabel('E  (V)', fontsize=10)
            ax.set_title(lbl, fontsize=10, fontweight='bold', color='#8a2c2c')
            ax.grid(True, alpha=0.25)
        fig.suptitle(
            f'All xi pulses — K&C space  |  colour = V_eq  |  tau = {tau:.0f} s',
    ...
    def _draw_overpot_curves(self, active):
        """All pulse overpotentials overlaid, coloured by V_eq — mirror of _draw_all_pulses.
        eta(t) = V_pulse(t) - V_eq_prev, plotted vs time since pulse start.
        """
        import matplotlib.cm as cm, matplotlib.colors as mcolors
        C_ANAL = '#5c3380'
        ...
        for name in active:
            ...
            for ax, cyc, cmap_name in [
                ...
                bl = [b for b in s['blocks'] if b['cycle'] == cyc
                      and len(b['_Vp']) >= 2]
                if not bl: continue
                veq_vals = [b.get('V_eq', float('nan')) for b in bl]
                valid_v  = [v for v in veq_vals if np.isfinite(v)]
                vmin = min(valid_v) if valid_v else 0.
                vmax = max(valid_v) if valid_v else 1.
                cmap = cm.get_cmap(cmap_name)
                norm = mcolors.Normalize(vmin=vmin, vmax=vmax)
                for i, b in enumerate(bl):
                    ...
                    veq = b.get('V_eq', float('nan'))
                    c   = cmap(norm(veq)) if np.isfinite(veq) else '#aaa'
                    # Reference = V_eq of PREVIOUS block (same direction)
                    v_ref = bl[i-1].get('V_eq', Vp[0]) if i > 0 else Vp[0]
                    eta_mV = (Vp - v_ref) * 1000.
                    ax.plot(t_rel, eta_mV, '-', lw=0.8, color=c, alpha=0.6)
        ...
        for ax, lbl in [(ax_d, 'Discharge'), (ax_c, 'Charge')]:
            ax.set_xlabel('Time since pulse start  (s)', fontsize=10)
            ax.set_ylabel(r'$\eta = V(t) - V_\mathrm{eq,prev}$  (mV)', fontsize=10)
            ax.set_title(lbl, fontsize=10, fontweight='bold', color=C_ANAL)
            ax.grid(True, alpha=0.25)
        fig.suptitle('Overpotential vs time  |  all pulses overlaid  |  colour = V_eq',
                     fontsize=10, fontweight='bold', color=C_ANAL)
        fig.tight_layout()
        self._flush('overpot_curves')

    def _draw_relax_curves(self, active):
        """All relaxation curves overlaid, coloured by V_eq — 2 rows x 2 cols.
        Top: V(t) since pulse end (absolute). Bottom: V-V_eq (convergence to 0).
        """
        import matplotlib.cm as cm, matplotlib.colors as mcolors
        C_ANAL = '#5c3380'
        ...
        ax_d  = fig.add_subplot(221); ax_c  = fig.add_subplot(222)
        ax_dd = fig.add_subplot(223); ax_cc = fig.add_subplot(224)

        for name in active:
            s = self._samples[name]
            for ax_v, ax_dv, cyc, cmap_name in [
                    (ax_d,  ax_dd, 'discharge', 'Blues_r'),
                    (ax_c,  ax_cc, 'charge',    'Oranges')]:
                bl = [b for b in s['blocks'] if b['cycle'] == cyc
                      and len(b['_Vr']) >= 2]
                if not bl: continue
                veq_vals = [b.get('V_eq', float('nan')) for b in bl]
                ...
                for b in bl:
                    ...
                    veq   = b.get('V_eq', float('nan'))
                    c     = cmap(norm(veq)) if np.isfinite(veq) else '#aaa'
                    ax_v.plot(t_rel, Vr, '-', lw=0.8, color=c, alpha=0.6)
                    if np.isfinite(veq):
                        ax_dv.plot(t_rel, (Vr - veq) * 1000., '-',
                                   lw=0.8, color=c, alpha=0.6)
                ax_dv.axhline(0, color='#555', lw=0.8, ls='--', alpha=0.5)
                for ax in [ax_v, ax_dv]:
                    sm = cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([])
                    cb = fig.colorbar(sm, ax=ax, fraction=0.03, pad=0.02)
                    cb.set_label(r'$V_\mathrm{eq}$ (V)', fontsize=8)

        ylabel_v  = r'$E_\mathrm{WE/RE}$  /  V vs. ref'
        ylabel_dv = r'$E - V_\mathrm{eq}$  (mV)'
        for ax, lbl, yl in [(ax_d,  'Discharge', ylabel_v),
                             (ax_c,  'Charge',    ylabel_v),
                             (ax_dd, 'Discharge', ylabel_dv),
                             (ax_cc, 'Charge',    ylabel_dv)]:
            ax.set_xlabel('Time since pulse end  (s)', fontsize=10)
            ax.set_ylabel(yl, fontsize=10)
            ax.set_title(lbl, fontsize=10, fontweight='bold', color=C_ANAL)
            ax.grid(True, alpha=0.25)
        fig.suptitle(
            'Relaxation curves  |  all pulses overlaid  |  colour = V_eq  '
            '|  top: V(t), bottom: V - V_eq',
            fontsize=10, fontweight='bold', color=C_ANAL)
        fig.tight_layout()
    ...
    def _draw_relax_delta(self, active):
        ...
        for ax, lbl, xlbl in [
            ...
            ax.set_ylabel(r'$\Delta V_\mathrm{relax}$ = $V_\mathrm{eq} - V_\mathrm{relax,0}$  /  V',
                          fontsize=10)
            ax.set_title(f'{lbl} - relaxation dV vs SOD/SOC',
                         fontsize=11, fontweight='bold', color=C_ANAL)
            ax.legend(fontsize=9, framealpha=0.95, edgecolor='#bbb')
        fig.suptitle(
            'Relaxation dV  =  V_eq - V_relax_start  |  voltage recovery during rest',
            fontsize=11, fontweight='bold', color=C_ANAL)
        fig.tight_layout()
        self._flush('relax_delta')

    def _draw_overpotential(self, active):
        """Overpotential eta and R_tot vs SOD/SOC using x_block for x-axis."""
        ...
        for name in active:
            ...
            for cyc, ax_eta, ax_R in [
                ...
                for b in bl:
                    sod = b.get('x_block', float('nan')) * 100.
                    if not np.isfinite(sod):
                        prev_veq = b['V_eq']; continue
        ...
        R_ylabel = r'$R_{\mathrm{tot}} \times m$  /  \u03a9\u00b7g'
        ...
        for ax, title, xl in [
            ...
            ax.set_title(title, fontsize=11, fontweight='bold', pad=6, color=C_ANAL)
            ax.legend(fontsize=9, framealpha=0.95, edgecolor='#bbb')
        ...
        mass_note = '  (R x mass, mass normalised)' if mass_any > 0 else '  (mass not set)'
        fig.suptitle('Overpotential = V_pulse_end - V_eq_prev   |   Rtot = eta / I' + mass_note,
                     fontsize=11, fontweight='bold', y=1.01, color=C_ANAL)
        fig.tight_layout()
        self._flush('overpot')

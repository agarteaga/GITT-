...
class GITTApp:
    ...
    def _build_equations_tab(self, parent):
        ...
        steprow(c,'B','Finite difference ΔE/Δx (block-to-block)',
                ...
                'Plotted vs E_eq (same x-axis as the top row and the smooth mode). '
        ...
        steprow(c,4,'D, dE/dx and OCV all assigned to xₙ (after pulse n)',
                'All computed quantities are plotted at xₙ. '                'In the Slopes & dE/dx tab BOTH rows (|dE/d√t| and |dE/dx|) are shown vs E_eq, '                'making it easy to compare slope and thermodynamic factor at the same voltage. '                'D plots show top row vs E_eq and bottom row vs SOD/SOC %.')
        sp(4)
        fig4 = sfig(9, 2.6)
        ...
        param_card('dE/dx mode  (radio button)', 'Differential (smooth dV/dx)', 'Differential | Finite diff',
            ...
            'Both modes plot |dE/dx| vs E_eq (same x-axis as the top row). '
        ...
        # ── 7. File format ────────────────────────────────────────────────────
        sec('7.  Input File Format')
        c = card()
        ctitle(c, 'Recommended: EC-Lab single-file ASCII (.txt)', DBLUE)
        body(c,
            'The simplest workflow: export one .txt file per sample from EC-Lab. '
            'The file contains everything — time, voltage, current, capacity — plus '
            'all sample metadata in the header (mass, Mw, electrode area, etc.). '
            'Just place the .txt file in its own folder and point the app at that folder. '
            'No separate cap file, no manual mass entry: everything is read automatically.',
            GRAY)
        steprow(c, 1, 'EC-Lab header (first ~111 lines)',
                'Lines of the form "Key : value" supply: '
                'mass of active material (mg), molecular weight, atomic weight of '
                'intercalated ion, number of electrons, battery capacity (Qmax), '
                'electrode area, and volume. '
                'The app reads these automatically and pre-fills all parameters.')
        steprow(c, 2, 'Column header line  (line N, where N = "Nb header lines")',
                'Tab-separated column names. Required columns detected by keyword: '
                'time/s or elapsed → time;  Ewe/V or potential → voltage;  '
                'I/mA or current → current (converted mA → A internally);  '
                'Capacity/mA.h → cumulative capacity in mAh.')
        steprow(c, 3, 'Data rows (line N+1 onward)',
                'European comma decimal (1,23456E+005) handled automatically. '
                'Discharge = negative current (I < 0). Charge = positive current (I > 0). '
                'Multiple .txt files in a folder are concatenated in alphabetical order '
                'with a 1 s gap — useful for multi-segment experiments.')
        notebox(c,
            'All sample metadata embedded in the header is parsed automatically: '
            'mass, Qmax, area. The mass spinner is pre-filled from the file header '
            'and can be overridden if needed.', DBLUE)
        tk.Frame(c, bg=BDR, height=1).pack(fill='x', padx=14, pady=4)
        ctitle(c, 'Also supported: BioLogic two-file format (.dat)', GRAY)
        body(c,
            'The classic BioLogic export produces two separate files per experiment: '
            'one containing time/voltage/current (*time*.dat) and one containing '
            'capacity/voltage (*cap*.dat). Place both in the same folder. '
            'Columns are detected by keyword matching; any column order is accepted. '
            'Comma decimal separators are handled automatically.', GRAY)
        for kw, desc in [
            ('Potential / Voltage / Ewe / Ecell / E/V', 'Voltage [V]'),
            ('Current / Iwe / I/A',                      'Current [A]  (already in A, not mA)'),
            ('Time / Elapsed',                            'Elapsed time [s]'),
            ('Capacity / mAh / mA.h / Charge',           'Cumulative capacity [mAh or mAh/g]'),
        ]:
            mpl_text(c, f'  •  {kw}  →  {desc}', BLK, CARD, fs=10, padx=28, pady=1)
        body(c,
            'Pulse duration tau is auto-detected in both formats as the median pulse '
            'length from the current signal. A 5% threshold separates pulses from '
            'OCP rest periods, tolerating small instrument leakage currents during rest.',
            GRAY)
        sp(4)

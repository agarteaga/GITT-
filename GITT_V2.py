...
class GITTApp:
    ...
    def _build_equations_tab(self, parent):
        ...
        steprow(c, 3, 'Relax kinetics map — 2D heatmap (operando-XRD style)',
                'x = log₁₀(τ / s), y = V_eq (V), colour = normalised process fraction. '
                'One panel per direction per sample. '
                'Blocks sorted by V_eq so the y-axis is monotonic. '
                'See Section 5b below for full description.')
        sp(4)

        # ── 5b. Relax kinetics map ────────────────────────────────────────────
        sec('5b.  Relax Kinetics Map  —  Operando-XRD-Style 2D Heatmap')
        c = card()
        ...
        body(c,
            ...
            'The Relax kinetics map is the electrochemical equivalent: '
            'x = log₁₀(τ), y = V_eq, colour = process strength at that (τ, V_eq) point.')
        body(c,
    ...
    def _draw_gitt_full(self, active, tfs, tfe, vlo, vhi):
        ...
        fig.set_size_inches(*self._figsize(2, n))
    ...
    # ── 2D relaxation kinetics map — dV/d(log t) vs V_eq × log t ────────────
    def _draw_relax_map(self, active):
        """Operando-XRD-style 2D heatmap of dV/d(log t).

        For each sample: one panel per direction (discharge | charge).
        x-axis : log₁₀(t_relax / s)   — the time-constant axis
        y-axis : V_eq (V)              — the electrochemical state axis
        colour : |dV/d(log t)| (mV/dec) — intensity = process strength

        A persistent process (same τ across all V_eq) appears as a vertical
        band at constant x.  A V_eq-dependent process shifts diagonally.
        Discharge and charge are plotted together on the same colour scale so
        the same τ in both directions shows up at the same colour intensity.
        """
        import matplotlib.cm as cm
        import matplotlib.colors as mcolors
        import matplotlib.gridspec as gridspec
        ...
        fig.set_size_inches(*self._figsize(2, n, row_h=4.2, col_w=5.5))
        gs = gridspec.GridSpec(2, n, figure=fig, hspace=0.40, wspace=0.35,
        ...
        for ci, name in enumerate(active):
            ...
            for ri, (cyc, lbl) in enumerate([('discharge', 'Discharge'),
                ...
                # Build grid: rows = blocks (ordered by V_eq), cols = log-time
                # Sort by V_eq so the y-axis is monotonic (like the XRD time axis)
                bl_sorted = sorted(bl, key=lambda b: b['V_eq'],
                                   reverse=(cyc == 'discharge'))

                veq_vals = [b['V_eq'] for b in bl_sorted]
                grid_rows = []

                # Common log-time axis: span all blocks
                lt_mins = []; lt_maxs = []
                for b in bl_sorted:
                ...
                for b in bl_sorted:
                    ...
                    if log_t is None:
                        row = np.zeros(N_RESAMP)
                    else:
                        row = np.interp(lt_ref, log_t, dvd_sm, left=0., right=0.)
                    grid_rows.append(row)

                Z = np.array(grid_rows)   # shape: (n_blocks, N_RESAMP)

                # Normalize each row by its total area under the curve so the
                # colour encodes the *fraction* of total relaxation signal at
                # each τ — making comparisons across different V_eq fair even
                # when the absolute dV/d(log t) amplitude varies strongly.
                row_areas = Z.sum(axis=1, keepdims=True)
                row_areas[row_areas < 1e-12] = 1.  # avoid division by zero
                Z_plot = Z / row_areas              # each row sums to 1

                # pcolormesh: x = log-time, y = V_eq
                y_vals = np.array(veq_vals)
                # Build cell edges between V_eq values
                if len(y_vals) > 1:
                    edges = np.concatenate([
                        [y_vals[0] - (y_vals[1] - y_vals[0]) / 2],
                        (y_vals[:-1] + y_vals[1:]) / 2,
                        [y_vals[-1] + (y_vals[-1] - y_vals[-2]) / 2]
                    ])
                else:
                    edges = np.array([y_vals[0] - 0.05, y_vals[0] + 0.05])

                # With shading='flat', X and Y must have one extra point each.
                lt_edges = np.append(lt_ref, lt_ref[-1] + (lt_ref[-1] - lt_ref[-2]))
                X_edge, Y_edge = np.meshgrid(lt_edges, edges)

                # Colour scale: 97th percentile of normalised values
                vmax = float(np.percentile(Z_plot[Z_plot > 0], 97)) if np.any(Z_plot > 0) else 1.
                vmin = 0.

                # Colormap matching operando XRD style: blue→cyan→green→yellow→red
                ...
                pcm = ax.pcolormesh(X_edge, Y_edge, Z_plot,
                                    cmap=xrd_cmap, vmin=vmin, vmax=vmax,
                                    shading='flat')
                cb = fig.colorbar(pcm, ax=ax, fraction=0.04, pad=0.02)
                cb.set_label(r'Norm. $|dV/d\log t|$ (frac. area)', fontsize=7)

                ax.set_xlabel(r'$\log_{10}(t_\mathrm{relax}\ /\ \mathrm{s})$', fontsize=9)
                ax.set_ylabel(r'$E_\mathrm{eq}$  (V)', fontsize=9)
                ax.set_title(f'{name}  |  {lbl}', fontsize=9,
                             fontweight='bold', color=col)
                ax.tick_params(labelsize=8)
                ax.grid(False)
        ...
        fig.suptitle(r'$dV/d\log t$ map  |  x = log τ,  y = $E_\mathrm{eq}$,  '
                     r'colour = normalised process fraction (row area)  |  vertical band = persistent τ',
                     fontsize=9, fontweight='bold', color=C_ANAL)
    ...
    def _export_figs(self):
        ...
        figs=[('gitt_curves.png','gitt_full'),('all_pulses.png','all_pulses'),
              ...
              ('relax_kinetics.png','relax_kinetics'),
              ('relax_kinetics_pp.png','relax_kinetics_pp'),
              ('relax_map.png','relax_map')]

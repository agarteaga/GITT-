...
def load_cap_segments(files, mass_mg=None, td=None):
    ...
    for path in sorted(files):
        ...
        for seg_end in seg_ends:
            ...
            if hc_signs and seg_idx < len(hc_signs):
                direction='discharge' if hc_signs[seg_idx] < 0 else 'charge'
            else:
                # Fallback: voltage trend (works for anodes, wrong for cathodes)
                direction='discharge' if sV[-1]<sV[0] else 'charge'
    ...
    if disc_parts_Q:
        ...
        disc={'Q':dQ,'V':dV,'Q_max':float(dQ[-1]),'unit':unit_final}
    else:
        disc={'Q':np.zeros(0),'V':np.zeros(0),'Q_max':0.,'unit':unit_final}

    if chg_candidates:
        best=max(chg_candidates,key=lambda s:s['Q_max'])
        chg={'Q':best['Q'],'V':best['V'],'Q_max':best['Q_max'],'unit':unit_final}
    else:
        chg=None
...
def build_dedx(disc):
    ...
    return interp1d(Vs_sm[valid],dEdx[valid],bounds_error=False,
                    fill_value=np.nan,kind='linear'),Q_max
...
def build_dedx_from_blocks(blocks):
    ...
    def _make(blist, descending=True):
        ...
        return interp1d(Vs_sm,dEdx,bounds_error=False,fill_value=np.nan,kind='linear')
...
class GITTApp:
    ...
    def _build_ui(self):
        ...
        self.tabs={}
        tab_names=[
            ('gitt_full',  'GITT curve'),
            ('all_pulses', 'All pulses (sqt)'),
            ('sqrt_fits',  'Conv: sqrt(t) fits  <Pulse#'),
            ('dEdx',       'Conv: slopes & dE/dx'),
            ('D_conv',     'Conv: D coefficient'),
            ('kc_fits',    'K&C: xi fits  <Pulse#'),
            ('kc_slopes',  'K&C: xi slopes'),
            ('D_kc',       'K&C: D coefficient'),
            ('ocv',        'OCV'),
        ]
        for key,label in tab_names:
            ...
            toolbar.update(); canvas.get_tk_widget().pack(fill='both',expand=True)
            self.tabs[key]={'fig':fig,'canvas':canvas}

        # ── Equations reference tab (pure Tk, no matplotlib) ──────────────────
        eq_frame=tk.Frame(self.nb,bg='white')
        self.nb.add(eq_frame,text='Equations')
    ...
    def _build_equations_tab(self, parent):
        ...
        mpl_text(hf, 'GITT Analysis — Equations & Methods Reference',
        ...
        # ── 1. Why relaxation ────────────────────────────────────────────────
        sec('1.  Why Relaxation?  —  The Pulse Contamination Problem')
        c = card()
        ctitle(c, 'Pulse analysis (D_conv  —  conventional)', RED)
        body(c, 'During the galvanostatic pulse, two contributions are mixed and cannot be separated:')
        render_eq(c,
            r'V(t)_{\rm pulse} \;=\; V_{\rm eq} \;+\; I \cdot R_{\rm tot}(t) \;+\; \mathrm{diffusion\ signal}',
        ...
        ax1.set_ylabel(r'$V - V_0$  (V)',fontsize=11)
        ax1.set_title('Pulse  (D_conv)',fontsize=11,fontweight='bold')
        ...
        ax2.set_ylabel(r'$V - V_{\rm eq}$  (V)',fontsize=11)
        ax2.set_title('Relaxation  (D_kc)',fontsize=11,fontweight='bold')
        ...
        # ── 2. Conventional ──────────────────────────────────────────────────
        sec('2.  Conventional Method  —  D_conv  (Weppner & Huggins 1977, Eq. 2)')
        c = card()
        ...
        body(c,
            'Note: the t=0 point (IR-drop jump) is always excluded from the fit '
        ...
        # ── 4. dE/dx ─────────────────────────────────────────────────────────
        sec('4.  Thermodynamic Factor  dVeq/dx  from OCV')
        c = card()
        ctitle(c, 'How the app computes dE/dx', DBLUE)
        ...
        steprow(c,2,'Normalise:  x = Q / Q_max  (0→1)',
                'Q_max from max discharge, max charge, or user-supplied theoretical capacity (C_th). '
                'Sort, deduplicate, smooth with Savitzky-Golay filter.')
        steprow(c,3,'Differentiate numerically',
                'dVeq/dx = gradient(V_smooth) / gradient(x)  →  interpolated as f(V_eq) for lookup at each block.')
        steprow(c,4,'Rebuild from equilibrium V_eq values (spike-free)',
                'Once block V_eq values are available, dE/dx is rebuilt from clean OCV points '
                'rather than the raw cap file (which contains pulse artefacts).')
        sp(4)
        fig4 = sfig(9, 2.6)
        ...
        param_card('Mass [mg]  per sample',
            'user-set; auto-enables only for mAh files', '0.1 – 200 mg',
            'Computing C_th default in mAh/g when cap file is in mAh',
            'C_th [mAh/g] = Q_max [mAh] / (mass [mg] / 1000). '
            'Greyed out automatically when cap file is already in mAh/g '
            '(mass not needed since Q/Q_max is mass-independent in both units).',
            'Set to active material mass. '
            'C_th spinner updates automatically when mass changes.', GRN)
    ...
    def _scan_data_dir(self):
        ...
        for name in samples:
            ...
            mass_lbl=tk.Label(sf,text='m (for C_th):',bg='white',fg='#888',
                     font=('sans',8))
            ...
            tk.Label(sf,text='Qmax:',bg='white',fg='#555',
                     font=('sans',8,'bold')).pack(side='left',padx=(4,1))
            qn_var=tk.StringVar(value='max discharge')
            qn_btn=tk.Button(sf,textvariable=qn_var,width=14,
                             font=('sans',8),relief='groove',bg='#e8f0fe',fg='#1a56cc',
                             command=lambda v=qn_var,n=name: self._toggle_qnorm(v,n))
            qn_btn.pack(side='left',padx=(0,2))
            # Theoretical capacity (used when mode = 'theoretical capacity')
            th_var=tk.DoubleVar(value=110.0)
            tk.Label(sf,text='C_th:',bg='white',fg='#888',font=('sans',8)).pack(side='left',padx=(4,0))
            tk.Spinbox(sf,textvariable=th_var,from_=1.,to=5000.,increment=10.,
                       width=6,font=('sans',9),command=self._replot_debounced).pack(side='left')
            tk.Label(sf,text='mAh/g',bg='white',fg='#888',font=('sans',8)).pack(side='left',padx=(1,4))
            self._chk_vars[name]=var
            self._chk_widgets[name]=sf
            self._sample_controls[name]={'mass':mass_var,'q_norm':qn_var,
                                         'qn_btn':qn_btn,'th_cap':th_var,
    ...
    def _on_pulse_change(self):
        ...
        try:
            cur_tab = self.nb.select()
            cur_text = self.nb.tab(cur_tab, 'text')
        except Exception:
            cur_tab = None; cur_text = ''
        ...
        # _draw_sqrt_fits and _draw_kc_fits already call _flush (synchronous draw)
        # Just ensure the UI processes pending events
        self.root.update_idletasks()
        # Stay on whatever tab the user is on — only switch if they are
        # NOT already on a fit tab (so changing pulse auto-navigates to fits)
        # FIX Bug 5: use substrings that actually appear in the full tab labels
        fit_texts = ['Conv: sqrt(t) fits', 'K&C: xi fits']
        on_fit_tab = any(t in cur_text for t in fit_texts)
        if not on_fit_tab:
            # Navigate to the most relevant fit tab
            try:
                self.nb.select(self.tabs['kc_fits']['canvas'].master)
            except Exception:
                pass
        else:
            # Stay on current tab and ensure it redraws
            try:
                self.nb.select(cur_tab)
            except Exception:
                pass
    ...
    def _toggle_qnorm(self, var, name):
        ...
        if 'qn_btn' in ctrl: ctrl['qn_btn'].config(bg=bg,fg=fg)
        self._replot()
    ...
    def _load_sample(self,name):
        ...
        try:
            ...
            _,_,_,cap_segs=load_cap_with_segs(cf,mass_mg=mass)
            self._samples[name]={'td':td,'disc':disc,'chg':chg,
                                  'cap_unit':cu,'cap_segs':cap_segs,
                                  'blocks':[],'dedx':None}
    ...
    def _replot(self,*_):
        ...
        try:
            ...
            for name in active:
                ...
                ctrl=self._sample_controls.get(name,{})
                mass=ctrl['mass'].get() if 'mass' in ctrl else self.mass_mg.get()
                q_norm=ctrl['q_norm'].get() if 'q_norm' in ctrl else 'max discharge'
                th_cap=ctrl['th_cap'].get() if 'th_cap' in ctrl else 110.0
                disc,chg,cu=load_cap_segments(cf,mass_mg=mass,td=s['td'])
                # When file is in mAh, keep C_th spinner in sync with current mass:
                # C_th [mAh/g] = Q_max [mAh] / (mass [mg] / 1000)
                # This way changing mass immediately updates the C_th default.
                if cu == 'mAh' and mass > 0:
                    q_best = max(disc['Q_max'], chg['Q_max'] if chg else 0.)
                    if q_best > 0:
                        cth_live = round(q_best / (mass / 1000.), 1)
                        try:
                            ctrl['th_cap'].set(cth_live)
                            th_cap = cth_live
                        except Exception:
                            pass
                # Single q_max_val from user choice
                if q_norm=='max charge' and chg and chg['Q_max']>0:
                    q_max_val=chg['Q_max']
                elif q_norm=='theoretical capacity' and mass>0 and th_cap>0:
                    q_max_val=mass*th_cap/1000.
                else:
                    q_max_val=disc['Q_max'] if disc['Q_max']>0 else 1.
                _,_,_,cap_segs=load_cap_with_segs(cf,mass_mg=mass)
                s['disc'],s['chg'],s['cap_unit'],s['cap_segs']=disc,chg,cu,cap_segs
                s['q_norm']=q_norm; s['q_max_val']=q_max_val
                ...
                for b in s['blocks']:
                    ...
                    if (np.isfinite(b['sl_sqrt']) and abs(b['sl_sqrt'])>1e-10
                        ...
                        b['D_conv']=np.nan
                    # Recompute D_kc using the updated dEdx and xi-windowed sl_kc
                    sl_kc=b.get('sl_kc',np.nan)
                    dEdx_kc=b.get('dEdx',np.nan)
                    # FIX Bug 4: if dEdx is NaN (D_conv path was skipped),
                    # fall back to a direct interpolator lookup for D_kc
    ...
    def _draw_all_pulses(self,active,tfs,tfe):
        ...
        ax_d=fig.add_subplot(121); ax_c=fig.add_subplot(122)
        for name in active:
            s=self._samples[name]; col=self._color(name)
            for ax,cyc in [(ax_d,'discharge'),(ax_c,'charge')]:
                for b in [b for b in s['blocks'] if b['cycle']==cyc]:
                    sq=b['_sqrt_t']; Vp=b['_Vp']; fm=b['_fm']
                    dV=Vp-Vp[0]; nz=b['_t_rel']>0
                    ax.plot(sq[nz],dV[nz],'-',lw=0.7,color=col,alpha=0.3)
                    if fm.sum()>=2:
                        ax.plot(sq[fm],dV[fm],'-',lw=2,color=col,alpha=0.7)
                        sl=b['sl_sqrt']
                        if np.isfinite(sl):
                            xf=np.linspace(sq[fm].min(),sq[fm].max(),50)
                            ic=np.mean(dV[fm])-sl*np.mean(sq[fm])
                            ax.plot(xf,sl*xf+ic,'--',lw=1,color=RED,alpha=0.35)
        for ax,lbl in [(ax_d,'Discharge'),(ax_c,'Charge')]:
            if tfs < tfe:  # guard: skip span if window is inverted or zero-width
            ...
            ax.set_xlabel('sqrtt (s½)'); ax.set_ylabel('ΔV (V)')
            ax.set_title(lbl,fontsize=10); ax.grid(True,alpha=0.25)
        # Legend
        from matplotlib.lines import Line2D
        fig.legend(handles=[Line2D([0],[0],color=self._color(n),lw=2,label=n)
                            for n in active],
                   loc='lower center',ncol=len(active),fontsize=8,
                   bbox_to_anchor=(0.5,0))
        fig.suptitle(f'All pulses in sqrtt space  |  green=[{tfs:.0f}–{tfe:.0f}s]',
                     fontsize=10,fontweight='bold')
        fig.tight_layout(rect=[0,0.06,1,1]); self._flush('all_pulses')
    ...
    def _draw_sqrt_fits(self,active,tfs,tfe,pulse_idx=None):
        ...
        for ci,name in enumerate(active):
            ...
            for row,(blist,cyc_lbl) in enumerate([(disc_bl,'Discharge'),(chg_bl,'Charge')]):
                ...
                nz=t_rel>0
                ax.plot(sq[nz],Vp[nz],'.',ms=4,color='#cccccc',alpha=0.8,
                        zorder=1,label='all points')
                if fm.sum()>=2:
                    ax.plot(sq[fm],Vp[fm],'o',ms=6,color=col,
                    ...
                    if np.isfinite(sl):
                        ...
                        ic=np.mean(Vp[fm])-sl*np.mean(sq[fm])
                ...
                ax.set_ylabel('E  (V)',fontsize=8)
                veq=b.get('V_eq',np.nan)
                veq_str=f'  Veq={veq:.4f}V' if np.isfinite(veq) else ''
    ...
    def _draw_kc_fits(self,active,tau,pulse_idx=None,xi_min=0.,xi_max=99.):
        ...
        for ci,name in enumerate(active):
            ...
            for row,(blist,cyc_lbl) in enumerate([(disc_bl,'Discharge'),(chg_bl,'Charge')]):
                ...
                ax.plot(xkc,Vr,'o',ms=4,color=col,alpha=0.85,
                        markeredgecolor='none',label='relaxation data')
                # Re-fit within user xi window
                xi_lo=max(xi_min,0.); xi_hi=min(xi_max,99.)
                ...
                if np.isfinite(sl_w):
                    ...
                    ax.plot(xf,sl_w*xf+ic_w,'-',color=col,lw=2.0,zorder=6,
                            label=f'R2={r2_w:.3f}  s={sl_w:+.5f}')
                    if xi_hi<30. or xi_lo>0.:
                        ax.axvspan(xi_lo,min(xi_hi,xkc.max()),alpha=0.10,
                                   color=col,zorder=0,label='fit range')
                ax.invert_xaxis()
                ax.set_xlabel('xi = sqrt(t+tau)-sqrt(t)  (s^0.5)  -> time ->',fontsize=7)
                ax.set_ylabel('E  (V)',fontsize=8)
                veq=b.get('V_eq',np.nan)
                veq_str=f'  Veq={veq:.4f}V' if np.isfinite(veq) else ''
                lbl='Discharge' if cyc_lbl=='Discharge' else 'Charge'
    ...
    def _draw_dedx(self,active,vlo,vhi):
        ...
        for name in active:
            ...
            for ax_sl,ax_dx,cyc in [(ax1,ax3,'discharge'),(ax2,ax4,'charge')]:
                blist=[b for b in s['blocks'] if b['cycle']==cyc
                ...
                if bsl:
                    ...
                    ax_sl.semilogy()
                    ax_sl.scatter(veqs,vals,s=20,color=col,alpha=0.55,edgecolors='none',label=name)
                    if len(veqs)>4:
                        vv,sm=rmed(veqs,vals)
                        ax_sl.plot(vv,sm,color=col,lw=2)
                bdx=[b for b in blist if np.isfinite(b.get('dEdx',np.nan))]
                if bdx:
                    veqs=[b['V_eq'] for b in bdx]
                    vals=[abs(b['dEdx']) for b in bdx]
                    ax_dx.semilogy()
                    ax_dx.scatter(veqs,vals,s=20,color=col,alpha=0.55,edgecolors='none',label=name)
                    if len(veqs)>4:
                        vv,sm=rmed(veqs,vals)
                        ax_dx.plot(vv,sm,color=col,lw=2)
        for ax,lbl in [(ax1,'dE/dsqrt(t) — Discharge'),(ax2,'dE/dsqrt(t) — Charge'),
                       (ax3,'dE/dx — Discharge'), (ax4,'dE/dx — Charge')]:
            ax.set_xlabel('V_eq (V)'); ax.set_ylabel(lbl.split('—')[0].strip())
            ax.set_title(lbl,fontsize=9); ax.legend(fontsize=6)
            ax.grid(True,which='both',alpha=0.3)
        fig.suptitle('Intermediate slopes for conventional D',fontsize=10,fontweight='bold')
        fig.tight_layout(); self._flush('dEdx')
    ...
    def _draw_d(self,active,d_field,tab_key,vlo,vhi,mr2,r2_field,sig,title):
        ...
        for name in active:
            ...
            for ax,cyc in [(ax_d,'discharge'),(ax_c,'charge')]:
                ...
                veqs=[b['V_eq'] for b in good]; Ds=[b[d_field] for b in good]
                # ── sigma-clip on log10(D) using robust MAD estimator ─────────
                if sig > 0 and len(Ds) >= 4:
                    logD = np.log10(Ds)
                    med  = np.median(logD)
                    mad  = np.median(np.abs(logD - med))
                    std  = mad * 1.4826        # convert MAD -> robust std
                    keep = np.abs(logD - med) <= sig * max(std, 0.05)
                    veqs = [v for v,k in zip(veqs,keep) if k]
                    Ds   = [d for d,k in zip(Ds,  keep) if k]
                if not veqs: continue
                ax.semilogy()
                ax.scatter(veqs,Ds,s=22,color=col,alpha=0.6,edgecolors='none')
                if len(veqs)>4:
                    vv,dm=rmed(veqs,Ds)
                    ax.plot(vv,dm,color=col,lw=2.5,label=f'{name} n={len(veqs)}')
        sig_lbl=f'   sigma-clip={sig:.1f}σ' if sig>0 else ''
        for ax,lbl in [(ax_d,'Discharge'),(ax_c,'Charge')]:
            ax.set_xlabel(r'$E_{\mathrm{eq}}$ / V vs. Na$^+$/Na', fontsize=10)
            ax.set_ylabel(r'$\tilde{D}\cdot(S/v_{\mathrm{m}})^2$ / mol$^2$ s$^{-1}$', fontsize=9)
    ...
    def _draw_kc_slopes(self,active,vlo,vhi,mr2):
        ...
        for name in active:
            s=self._samples[name]; col=self._color(name)
            for ax,cyc in [(ax_d,'discharge'),(ax_c,'charge')]:
                # FIX Bug 2: apply mr2 filter on r2_kc so KC slopes are consistent with D_kc panel
                ...
                if bl:
                    ...
                    if len(veqs)>4:
                        vv,sm=rmed(veqs,vals)
                        ax.plot(vv,sm,color=col,lw=2.5,label=name)
        for ax,lbl in [(ax_d,'Discharge'),(ax_c,'Charge')]:
            ax.set_xlabel('V_eq (V)'); ax.set_ylabel('|slope_KC| (V s⁻½)')
            ax.set_title(lbl,fontsize=10); ax.legend(fontsize=7)
            ax.grid(True,which='both',alpha=0.3)
        fig.suptitle('K&C slope  ~  1/sqrtD',fontsize=10,fontweight='bold')
    ...
    def _draw_ocv(self, active):
        ...
        for name in active:
            ...
            if dbl and disc_s and disc_s['Q_max']>1e-6:
                Nd = len(dbl)
                xd_s = [i/max(Nd-1,1)*disc_s['Q_max']/Q_ref_s for i in range(Nd)]
                all_x_d.extend(xd_s)
                all_veq_d.extend(b['V_eq'] for b in dbl)
            if cbl and chg_s and chg_s['Q_max']>1e-6:
                Nc = len(cbl)
                xc_s = [i/max(Nc-1,1)*chg_s['Q_max']/Q_ref_s for i in range(Nc)]
                all_x_c.extend(xc_s)
                all_veq_c.extend(b['V_eq'] for b in cbl)

        # y-axis: from block V_eq values (more reliable than cap V range)
        all_veq = all_veq_d + all_veq_c
        if all_veq:
            v_min = min(all_veq); v_max = max(all_veq)
            margin_v = max((v_max - v_min) * 0.05, 0.1)
            y_lo = max(0., v_min - margin_v); y_hi = v_max + margin_v
            v_span = y_hi - y_lo
            major_step = 0.5 if v_span <= 3 else (1.0 if v_span <= 6 else 2.0)
            minor_step = major_step / 5
        else:
            y_lo, y_hi = 0.0, 5.0; major_step, minor_step = 0.5, 0.1

        # x-axis: extend to fit all data (charge can exceed 1.0 when Q_chg > Q_disc)
        x_max_d = max(all_x_d) if all_x_d else 1.0
        x_max_c = max(all_x_c) if all_x_c else 1.0
        x_hi_d  = round(x_max_d + 0.04, 2)
        x_hi_c  = round(x_max_c + 0.04, 2)
        ...
        def _style_ax(ax, x_hi):
            ...
            x_tick = 0.25 if x_hi <= 1.1 else (0.5 if x_hi <= 2.0 else 1.0)
            ax.xaxis.set_major_locator(ticker.MultipleLocator(x_tick))
            ax.xaxis.set_minor_locator(ticker.MultipleLocator(x_tick/5))
        ...
        for name in active:
            ...
            def _veq_to_x(bl, seg, cap_segs=None, is_discharge=True):
                """Map each block V_eq → x = Q(V_eq) / Q_ref.

                Builds a merged V→Q lookup from ALL same-direction cap segments
                (e.g. both discharge segments from split files), so out-of-range
                blocks are handled correctly without any reversed-sign heuristics.
                Falls back to linear extrapolation only beyond the merged range.
                """
                if not bl or seg is None or seg['Q_max'] < 1e-6:
                    return None, None
                veqs = np.array([b['V_eq'] for b in bl])

                # Build merged V→Q from all same-direction segments
                if cap_segs:
                    same_dir = [(sq, sv) for sq, sv, isd in cap_segs if isd == is_discharge]
                else:
                    same_dir = []

                if same_dir:
                    V_all = np.concatenate([sv for sq, sv in same_dir])
                    Q_all = np.concatenate([sq for sq, sv in same_dir])
                    idx_s = np.argsort(V_all); V_all = V_all[idx_s]; Q_all = Q_all[idx_s]
                    _, ui = np.unique(V_all, return_index=True)
                    V_merged = V_all[ui]; Q_merged = Q_all[ui]
                    # Normalise to [0, Q_seg_max] so x = q/Q_ref is consistent
                    Q_merged = Q_merged - Q_merged.min()
                else:
                    Vp = seg['V']; Qp = seg['Q']
                    idx_s = np.argsort(Vp); Vp = Vp[idx_s]; Qp = Qp[idx_s]
                    _, ui = np.unique(Vp, return_index=True)
                    V_merged = Vp[ui]; Q_merged = Qp[ui]

                # Interpolate / extrapolate
                q = np.interp(veqs, V_merged, Q_merged,
                              left=Q_merged[0], right=Q_merged[-1])
                q = np.clip(q, 0., None)
                return veqs, q / Q_ref
            # ── Discharge OCV (left panel) ──────────────────────────────────
            cap_segs = s.get('cap_segs', None)
            veqs, x = _veq_to_x(disc_bl, disc, cap_segs=cap_segs, is_discharge=True)
            if veqs is not None:
                order = np.argsort(x)
            ...
            veqs, x = _veq_to_x(chg_bl, chg, cap_segs=cap_segs, is_discharge=False)
            if veqs is not None:
                order = np.argsort(x)
        ...
        ax1.set_xlabel(r'$x = Q/Q_{\mathrm{ref}}$  (discharge: $0\rightarrow Q_{\rm disc}/Q_{\rm ref}$)', fontsize=10)
        ax1.set_ylabel(ylabel, fontsize=12)
        ax1.set_title('Discharge  (V_eq from relaxation endpoints)', fontsize=11, fontweight='bold', pad=6)
        if plotted_d:
            ax1.legend(fontsize=10, framealpha=0.95, edgecolor='#bbb',
                       handlelength=1.5, loc='upper right')

        ax2.set_xlabel(r'$x = Q/Q_{\mathrm{ref}}$  (charge: $0\rightarrow Q_{\rm chg}/Q_{\rm ref}$)', fontsize=10)
        ax2.set_ylabel(ylabel, fontsize=12)
        ax2.set_title('Charge  (V_eq from relaxation endpoints)', fontsize=11, fontweight='bold', pad=6)
        if plotted_c:
            ax2.legend(fontsize=10, framealpha=0.95, edgecolor='#bbb',
                       handlelength=1.5, loc='best')

        fig.suptitle(
            r'Open circuit voltage vs. $x = Q/Q_{\mathrm{max}}$',

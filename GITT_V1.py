...
def extract_blocks(td, tfs=60., tfe=540., min_relax=None, tau_s=600.):
    ...
    # ── Remove last charge block if it has no genuine relaxation ────────────
    # The final charge pulse in a GITT experiment typically ends without a
    # full relaxation period (the experiment stops or switches direction).
    # Symptom: the last charge block's V_eq drops BELOW the previous charge
    # block's V_eq (should be monotonically increasing for charge).
    chg_blocks = [b for b in blocks if b['cycle'] == 'charge']
    if len(chg_blocks) >= 2:
        last_c  = chg_blocks[-1]
        prev_c  = chg_blocks[-2]
        if last_c['V_eq'] < prev_c['V_eq']:
            blocks = [b for b in blocks if b is not last_c]
    return blocks
...
class GITTApp:
    ...
    def _build_ui(self):
        ...
        # ── Notebook (created before section bar so we can pack in right order) ─
        # Style ttk Notebook tabs using 'clam' theme which supports tab colours
        style = ttk.Style()
        try:
            style.theme_use('clam')
        except Exception:
            style.theme_use('default')
        # Create named styles for each section colour
        _SEC_STYLES = {
            'raw':  ('#2c5f8a', 'white'),
            'conv': ('#1a6b35', 'white'),
            'kc':   ('#8a2c2c', 'white'),
        ...
        for sec_key, (bg, fg) in _SEC_STYLES.items():
            sname = f'Sec.{sec_key}.TNotebook.Tab'
        ...
        # ── Section header bar above the notebook ──────────────────────────────
        # Simple coloured labels — one per section — with a bottom border
        # that matches the coloured top border on each tab frame.
        sec_bar = tk.Frame(self.root, bg='#e0e0e0', bd=0)
        sec_bar.pack(fill='x', padx=4, pady=(4,0))

        # Count tabs per section
        sec_counts = {}
        for _,_,sec in TAB_SECTIONS:
            sec_counts[sec] = sec_counts.get(sec, 0) + 1
        sec_counts['docs'] = 1   # Documentation tab

        all_sections = list(SECTIONS) # includes 'docs'
        for sec_key, sec_label, sec_dark, sec_light in all_sections:
            count = sec_counts.get(sec_key, 0)
            # Section label box
            lbl_fr = tk.Frame(sec_bar, bg=sec_dark, padx=1, pady=1)
            lbl_fr.pack(side='left', padx=(0,1), pady=0)
            tk.Label(lbl_fr, text=sec_label,
                     bg=sec_dark, fg='white',
                     font=('Helvetica','8','bold'),
                     padx=10, pady=3).pack()

        # ── Notebook tabs ───────────────────────────────────────────────────────
        # Map section key to colours for tab styling
        SEC_COLOUR = {s[0]: (s[2], s[3]) for s in SECTIONS}

        self.nb.pack(fill='both', expand=True, padx=4, pady=(0,4))

        for key, label, sec in TAB_SECTIONS:
            frame = tk.Frame(self.nb, bg='white')
            # Add coloured top border to each tab frame
            tk.Frame(frame, bg=SEC_COLOUR[sec][0], height=3).pack(fill='x', side='top')
            self.nb.add(frame, text=label)
            # Apply section colour style to this tab
            try:
                tab_idx = self.nb.index('end') - 1
                self.nb.tab(tab_idx, style=f'Sec.{sec}.TNotebook.Tab')
            except Exception:
                pass
        ...
        tk.Frame(eq_frame, bg='#4a4a4a', height=3).pack(fill='x', side='top')
        self.nb.add(eq_frame, text='Documentation')
        try:
            style.configure('Sec.docs.TNotebook.Tab',
                            background='#4a4a4a', foreground='white',
                            padding=[6,3], font=('Helvetica',8,'bold'))
            style.map('Sec.docs.TNotebook.Tab',
                      background=[('selected','#4a4a4a'),('active','#4a4a4a')],
                      foreground=[('selected','white'),('active','white')])
            self.nb.tab(self.nb.index('end')-1, style='Sec.docs.TNotebook.Tab')
        except Exception:
            pass
    ...
    def _replot(self,*_):
        ...
        try:
            ...
            for name in active:
                ...
                for cyc_blocks, q_ref_cyc in [
                    ([b for b in s['blocks'] if b['cycle']=='discharge'], q_max_disc),
                    ([b for b in s['blocks'] if b['cycle']=='charge'],    q_max_chg),
                ]:
                    x_cum = 0.; prev_veq = None
                    for b in cyc_blocks:
                        Tp = b['_Tp']
                        dt = Tp[-1] - Tp[0]
                        Q_pulse_mAh = abs(b['I_pulse']) * dt / 3600. * 1000.
                        delta_x = Q_pulse_mAh / q_ref_cyc if q_ref_cyc > 1e-9 else 0.
                        x_cum += delta_x
                        b['x_block'] = float(np.clip(x_cum, 0., 1.))
                        b['dEdx_fd'] = np.nan   # kept for compatibility
                        prev_veq = b.get('V_eq', np.nan)
                for b in s['blocks']:
    ...
    def _draw_dedx(self,active,vlo,vhi):
        ...
        for name in active:
            ...
            for ax_sl,ax_dx,cyc in [(ax1,ax3,'discharge'),(ax2,ax4,'charge')]:
                ...
                # slopes vs V_eq (top row — unchanged)
                ...
                # |dE/dx| vs SOD/SOC % (bottom row) — smooth differential
                # Uses build_dedx_from_blocks interpolator evaluated at each block's V_eq
                ...
                if bdx and dedx_fn is not None:
                    xs=[b['x_block']*100. for b in bdx]  # SOD/SOC %
                    try:
                        vals=[abs(float(dedx_fn(b['V_eq']))) for b in bdx]
                    ...
                    if vals:
                        ax_dx.semilogy()
                        ax_dx.scatter(xs,vals,s=20,color=col,alpha=0.55,edgecolors='none',label=name)
                        if len(xs)>3:
                            order=np.argsort(xs)
                            xs_s=[xs[i] for i in order]; vals_s=[vals[i] for i in order]
                            vv,sm=rmed(xs_s,vals_s)
                            ax_dx.plot(vv,sm,color=col,lw=2)
        xlabel_v = r'$E_{\mathrm{eq}}$ / V vs. ref'
        for ax,lbl,xl in [
                (ax1,'dE/d√t — Discharge', xlabel_v),
                (ax2,'dE/d√t — Charge',    xlabel_v),
                (ax3,'|dE/dx| — Discharge', 'SOD  (%)'),
                (ax4,'|dE/dx| — Charge',    'SOC  (%)')]:
            ax.set_xlabel(xl, fontsize=9)
            ax.set_ylabel(lbl.split('—')[0].strip(), fontsize=9)
            ax.set_title(lbl, fontsize=9); ax.legend(fontsize=6)
            ax.grid(True, which='both', alpha=0.3)
        fig.suptitle('Intermediate slopes and thermodynamic factor dE/dx',
                     fontsize=10, fontweight='bold')
    ...
    def _draw_overpotential(self, active):
        """Plot overpotential η and R_tot vs SOD/SOC for each sample.

        η_n     = V_pulse_end_n  -  V_eq_{n-1}
                  (voltage at last point of pulse n  minus  equilibrium OCV
                   before that pulse = after previous relaxation)

        R_tot_n = η_n / I_n   [Ω]
        R_norm  = R_tot_n × mass [g]   [Ω·g]   (normalised by active mass)

        SOD = cumulative Q_pulse / Q_max_disc × 100 %   (discharge)
        SOC = cumulative Q_pulse / Q_max_chg  × 100 %   (charge)
        """
        fig = self._clf('overpot')
        fig.set_size_inches(14, 10)
        ax1 = fig.add_subplot(221)   # η vs SOD
        ax2 = fig.add_subplot(222)   # η vs SOC
        ax3 = fig.add_subplot(223)   # R_tot vs SOD
        ax4 = fig.add_subplot(224)   # R_tot vs SOC

        for name in active:
            s   = self._samples[name]
            col = self._color(name)
            blocks    = s.get('blocks', [])
            q_max_d   = s.get('q_max_disc', s.get('q_max_val', 1.))
            q_max_c   = s.get('q_max_chg',  s.get('q_max_val', 1.))
            ctrl      = self._sample_controls.get(name, {})
            mass_g    = ctrl['mass'].get() / 1000. if 'mass' in ctrl else 0.

            for cyc, ax_eta, ax_R, q_max_cyc in [
                ('discharge', ax1, ax3, q_max_d),
                ('charge',    ax2, ax4, q_max_c),
            ]:
                ...
                sod_list = []; eta_list = []; R_list = []
                x_cum = 0.; prev_veq = None

                for b in bl:
                    dt   = b['_Tp'][-1] - b['_Tp'][0]
                    Qp   = abs(b['I_pulse']) * dt / 3600. * 1000.   # mAh
                    x_cum += Qp
                    sod  = x_cum / q_max_cyc * 100.   # %

                    if prev_veq is not None:
                        v_end = b['_Vp'][-1]          # V at END of pulse
                        eta   = v_end - prev_veq       # overpotential [V]
                        I     = b['I_pulse']
                        R_tot = eta / I if abs(I) > 1e-12 else np.nan
                        R_norm = R_tot * mass_g if mass_g > 0 else R_tot

                        sod_list.append(sod)
                        eta_list.append(eta * 1000.)    # → mV
                        R_list.append(R_norm if mass_g > 0 else R_tot)
                ...
                R_a   = np.array(R_list)
                valid = np.isfinite(R_a) & np.isfinite(eta_a)

                ax_eta.plot(sod_a[valid], eta_a[valid], 'o-',
                            ms=5, lw=1.5, color=col,
                            markeredgewidth=0, alpha=0.85, label=name)
                ax_R.plot(sod_a[valid], R_a[valid], 'o-',
                          ms=5, lw=1.5, color=col,
                          markeredgewidth=0, alpha=0.85, label=name)

        R_ylabel = r'$R_{\mathrm{tot}} 	imes m$  /  Ω·g' if True else r'$R_{\mathrm{tot}}$ / Ω'

        for ax, title, xl in [
            (ax1, 'Overpotential vs SOD',  'SOD  (%)'),
            (ax2, 'Overpotential vs SOC',  'SOC  (%)'),
            (ax3, 'Rtot vs SOD', 'SOD  (%)'),
            (ax4, 'Rtot vs SOC', 'SOC  (%)'),
        ]:
        ...
        ax1.set_ylabel('Overpotential / mV', fontsize=12)
        ax2.set_ylabel('Overpotential / mV', fontsize=12)
        ax3.set_ylabel(R_ylabel, fontsize=12)
        ax4.set_ylabel(R_ylabel, fontsize=12)

        # Check if mass is set — label accordingly
        ...
        mass_note = f'  (R × mass,  mass normalised)' if mass_any > 0 else '  (mass not set — showing R in Ω)'
        fig.suptitle('Overpotential = V_pulse_end − V_eq_prev   |   Rₜₒₜ = η / I' + mass_note,
